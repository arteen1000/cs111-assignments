...
struct hash_table_v1 {
  ...
  pthread_mutex_t coarse_grain;
};

/* allocate hash_table
 * initialize all entries to valid lists
 * return valid hash_table
 */
struct hash_table_v1 *hash_table_v1_create()
{
	struct hash_table_v1 *hash_table = calloc(1, sizeof(struct hash_table_v1));
	assert(hash_table != NULL);
	int rc = pthread_mutex_init(&hash_table->coarse_grain, NULL);
	if (rc != 0) _Exit(rc);
...
/* return address of entry that key would hash to */
                             ...
                             uint32_t value)
{
	...
	if (list_entry != NULL) {
		...
		err = pthread_mutex_unlock(&hash_table->coarse_grain);
		if (err) _Exit(err);
		return;
	}
...
void hash_table_v1_destroy(struct hash_table_v1 *hash_table)
{
    ...
    int rc = pthread_mutex_destroy(&hash_table->coarse_grain);
	if (rc != 0) _Exit(rc);
	for (size_t i = 0; i < HASH_TABLE_CAPACITY; ++i) {
		struct hash_table_entry *entry = &hash_table->entries[i];
