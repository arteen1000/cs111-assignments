...
struct hash_table_v2 *hash_table_v2_create()
{
	...
	for (size_t i = 0; i < HASH_TABLE_CAPACITY; ++i) {
		...
		int err = pthread_mutex_init(&entry->fine_grain, NULL);
		if (err) _Exit(err);
                             ...
                             uint32_t value)
{
	...
	if (list_entry != NULL) {
		...
		err = pthread_mutex_unlock(&hash_table_entry->fine_grain);
		if (err) _Exit(err);
		return;
	}
...
void hash_table_v2_destroy(struct hash_table_v2 *hash_table)
{
	for (size_t i = 0; i < HASH_TABLE_CAPACITY; ++i) {
		struct hash_table_entry *entry = &hash_table->entries[i];
		int err = pthread_mutex_destroy(&entry->fine_grain);
		if (err) _Exit(err);
